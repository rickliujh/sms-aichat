...
data "aws_iam_policy_document" "chat_ecr" {
  statement {
...
resource "aws_iam_role_policy_attachment" "github_actions_ecr_push_state_lock_policy" {
  role       = aws_iam_role.github_actions_ecr_push.name
  policy_arn = module.aws-tf-backend.state_file_iam_policy_arn
}
...
resource "github_actions_variable" "chat_ecr_repo_name" {
